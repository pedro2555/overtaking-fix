...
namespace OvertakingFix
{
    public class mainScript : Script
    {
        #region Properties
        internal static readonly string scriptName = "OvertakingFix";
            ...
            this.Tick += (s, e) =>
                {
                    try
                    {
                        if (Player.Character.isInVehicle() && Player.Character.CurrentVehicle.GetPedOnSeat(VehicleSeat.Driver) == Player.Character && Function.Call<bool>("IS_CAR_STOPPED", Player.Character.CurrentVehicle))
                            World.GetClosestVehicle(Player.Character.CurrentVehicle.GetOffsetPosition(new Vector3(0, -7, 0)), 4).GetPedOnSeat(VehicleSeat.Driver).Task.StandStill(110);
                    }
                    catch (NullReferenceException) { /* Expected exception */ }
                    catch (Exception crap)
                    {
                        Log("Tick", crap.Message);
                    }
                };
